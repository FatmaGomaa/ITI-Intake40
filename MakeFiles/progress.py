...
def main():
  BarMaxValue=int(sys.argv[1])
  if BarMaxValue > 0 and BarMaxValue < 75 :
    prog = progressBar(maxValue = BarMaxValue)
    file = open("progress.txt",'r')
    #file.seek(0)
    #progressValue=file.readline()
    progressValue=re.findall(r'\d+', file.read())
    progressValue=int(progressValue[0])
    file.close()
    prog.updateAmount(progressValue)
    print()
    prog.draw()
    print()
    sys.exit()
