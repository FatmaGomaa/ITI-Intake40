...
def extractFemaleNames(filename,letter):
  ...
  extractedNames = re.findall(r'<.*?>(\d+)<.*?><.*?>(\w+)<.*?><.*?>(\w+)<.*?>',file.read())
  temp = sorted(extractedNames, key=lambda girls: girls[2]) 
  #print(temp)
  file.close()
  for tuple in temp:
    if letter in tuple[2]:
      extractedGirlsNames.append(str(tuple[2] +' ' + tuple[0]))
  #print(extractedGirlsNames)
  return extractedGirlsNames
...
def extractMaleNames(filename,letter):
  ...
  file = open(filename,'r')
  extractedNames = re.findall(r'<.*?>(\d+)<.*?><.*?>(\w+)<.*?><.*?>(\w+)<.*?>',file.read())
  temp = sorted(extractedNames, key=lambda boys: boys[1], reverse = True)
  #print(temp)
  file.close()
  for tuple in temp:
    if letter in tuple[1]:
      extractedBoysNames.append(str(tuple[1] +' ' + tuple[0]))
  #print(extractedBoysNames)
  return extractedBoysNames
...
def extractDocAndPopularityDate(filename):
  ...
  extractedDate = []
  extractedName = []
  extractedNameAndDate = ''
  file = open(filename,'r')
  extractedDate = re.findall(r'<h1>([\w ]+)<.*?>([\w ,\d]+)<.*?>',file.read())
  file.seek(0)
  extractedName = re.findall(r'<\w.+"center">([\w ]+)<.*?>',file.read())
  file.close()
  extractedNameAndDate = str(extractedDate[0][1] + ' and ' + extractedName[0])
  return extractedNameAndDate
