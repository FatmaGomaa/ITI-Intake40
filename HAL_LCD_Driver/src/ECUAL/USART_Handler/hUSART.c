...
STD_ERROR UART_Init(UART_Frame_Cfg_t * cfg)
{
	...
	switch(cfg->UART_ID){
		case UART_ONE:
			...
			RCC_stdErrorControlPerihperal(APB2, RCC_APB2_CLOCK_UART1EN,  ON);

			break;
		case UART_TWO:
			RCC_stdErrorControlPerihperal(APB2, RCC_APB2_CLOCK_PORT_A,  ON);
			// RCC configuration for UART1
			RCC_stdErrorControlPerihperal(APB1, RCC_APB1_CLOCK_UART2EN,  ON);

			break;
		case UART_THREE:
			RCC_stdErrorControlPerihperal(APB2, RCC_APB2_CLOCK_PORT_B,  ON);
			// RCC configuration for UART1
			RCC_stdErrorControlPerihperal(APB1, RCC_APB1_CLOCK_UART3EN,  ON);
	...
	GPIO_STD_ERROR_writePin(UART_GPIO_RX.PORT, UART_GPIO_RX.PIN,  HIGH);
