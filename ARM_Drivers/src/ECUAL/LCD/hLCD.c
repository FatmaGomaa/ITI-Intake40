...
static void LCD_initProcess(void)
{
	...
	case function_set :
		...
		if(GPIO_u8getPinValue(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_Enable_PIN)){
			GPIO_writePin(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_Enable_PIN, LOW);
			Local_CurrentProcess = display_control;

		}else {
			GPIO_writePin(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_RS_PIN, LOW);
			GPIO_writePin(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_RW_PIN, LOW);
			for (Local_u8Iterator = 0; Local_u8Iterator < LCD_DATA_PINS_NUM; Local_u8Iterator ++)
			{
				...
				GPIO_writePin(APP_LCDs[0].LCD_Data_PORT, APP_LCDs[0].LCD_Data_Pins[Local_u8Iterator], Local_u8BitValue);

			}
			GPIO_writePin(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_Enable_PIN, HIGH);
		}
		break;
...
static void LCD_WriteProcess(void){
	...
	if(Local_u8Counter < Global_StringLenght){
		if(GPIO_u8getPinValue(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_Enable_PIN)){
			...
			GPIO_writePin(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_RS_PIN, HIGH);
			GPIO_writePin(APP_LCDs[0].LCD_CONTROL_PORT, APP_LCDs[0].LCD_RW_PIN, LOW);
			for (Local_u8Iterator = 0; Local_u8Iterator < LCD_DATA_PINS_NUM; Local_u8Iterator ++)
			{
