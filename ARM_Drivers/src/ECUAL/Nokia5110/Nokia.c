...
extern Nokia_Cfg_t Nokia;

static void writeCommand(u8 Command);
static void writeData(u8* Data, u16 size);

void Nokia_Init(void){
	GPIO_Config(&Nokia.RST);
	GPIO_Config(&Nokia.DC);
	GPIO_Config(&Nokia.CE);

	/*Reset Cycle */
	GPIO_writePin(Nokia.RST.PORT, Nokia.RST.PIN, HIGH);
	Delay_ms(2);
	GPIO_writePin(Nokia.RST.PORT, Nokia.RST.PIN, LOW);
	Delay_ms(2);
	GPIO_writePin(Nokia.RST.PORT, Nokia.RST.PIN, HIGH);
...
static void writeCommand(u8 Command){
	...
	GPIO_writePin(Nokia.CE.PORT,Nokia.CE.PIN, LOW);
	/*Select Command Mode*/
	GPIO_writePin(Nokia.DC.PORT,Nokia.DC.PIN, LOW);
	/*send data over SPI*/
	SPI_Send(&Command,1);
	/*deselect Nokia from slavery*/
	GPIO_writePin(Nokia.CE.PORT,Nokia.CE.PIN, HIGH);
...
static void writeData(u8* Data, u16 size){
	...
	GPIO_writePin(Nokia.CE.PORT,Nokia.CE.PIN, LOW);
	/*Select Data Mode*/
	GPIO_writePin(Nokia.DC.PORT,Nokia.DC.PIN, HIGH);
	/*send data over SPI*/
	SPI_Send(Data,size);
	/*deselect Nokia from slavery*/
	GPIO_writePin(Nokia.CE.PORT,Nokia.CE.PIN, HIGH);
